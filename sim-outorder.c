...
#include "machine.def"
      ...
      else
	{
      ...
      sim_total_insn++;
      if(sim_total_insn % (max_insts/5) == 0)
      {
        debug("Instruction %d: Reads in Clean Lines = %d", sim_total_insn, get_read_in_clean_lines());
        debug("Instruction %d: Reads in Dirty Lines = %d", sim_total_insn, get_read_in_dirty_lines());
      }
